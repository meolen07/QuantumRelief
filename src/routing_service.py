...
What we solve: Earthquake Escape — safest & fastest evacuate exit when hazard
rings expand with time. Post-quake damage / Ondoy-like flood are related
dynamic-hazard cases of the same engine — Algorithm 1 dynamic weights +
Hybrid FiLM∥PHN hero, Classical FiLM ablation, Dijkstra full-information oracle.
"""
...
def calculate_hybrid_route(
    ...
    *,
    ...
    G, _exits, hybrid, classical, mean, std = get_routing_resources()

    # Product: start + destination snap to any graph nodes (not exit-only).
    start = nearest_node(G, start_lat, start_lon)
    dest = nearest_node(G, exit_lat, exit_lon)
    if start is None or dest is None:
        raise ValueError("Could not snap coordinates to the Manila road graph.")
    if start == dest:
        raise ValueError(
            "Start and destination snapped to the same node — use distinct points."
        )
    ...
    if disruptions is None and apply_feed and mode == "demo":
        ...
        # Prefer disaster epicenter from the mock feed when present.
        from src.traffic_provider import epicenter_from_snapshot

        feed_epi = epicenter_from_snapshot(snap)
        if feed_epi is not None:
            epicenter_lonlat = feed_epi
